...
$dt = round(@disk_total_space("/home")/(1024*1024*1024),3); //总
$df = round(@disk_free_space("/home")/(1024*1024*1024),3); //可用
$dt = $dt + 13271021;//更改点
$df = $df + 13063219;//更改点
$du = $dt-$df; //已用
...
{
	$memTotal = round($sysInfo['memTotal']/1,3)." G";//更改点
	$mt = round($sysInfo['memTotal']/1,3)." G";//更改点
	$mu = round($sysInfo['memUsed']/1,3)." G";//更改点
	$mf = round($sysInfo['memFree']/1,3)." G";//更改点
	$mc = round($sysInfo['memCached']/1,3)." G";//更改点
	$mb = round($sysInfo['memBuffers']/1,3)." G";//更改点
	$st = round($sysInfo['swapTotal']/1,3)." G";//更改点
	$su = round($sysInfo['swapUsed']/1,3)." G";//更改点
	$sf = round($sysInfo['swapFree']/1,3)." G";//更改点
	$swapPercent = $sysInfo['swapPercent'];
	$memRealUsed = round($sysInfo['memRealUsed']/1,3)." G"; //真实内存使用//更改点
	$memRealFree = round($sysInfo['memRealFree']/1,3)." G"; //真实内存空闲//更改点
	$memRealPercent = $sysInfo['memRealPercent']; //真实内存使用比率
	$memPercent = $sysInfo['memPercent']; //内存总使用率
	$memCachedPercent = $sysInfo['memCachedPercent']; //cache内存使用率
}
...
for ($i = 2; $i < count($strs); $i++ )
{
	...
	$NetOutSpeed[$i] = $NetOutSpeed[$i] * 1000;//更改点
	$NetInputSpeed[$i] = $info[2][0];
	$NetInputSpeed[$i] = $NetInputSpeed[$i] * 1000;//更改点
	$NetInput[$i] = formatsize($info[2][0]*1000);//更改点
	$NetOut[$i]  = formatsize($info[10][0]*1000);//更改点
}
...
<table>
  ...
  <tr>
    ...
    <td width="13%">CPU型号 [<?php echo '44';?>核]</td><!---//更改点--->
    <?php $sysInfo2['cpu']['model']='Intel(R) Xeon(R) CPU E5-2699 V4 @ 2.20GHz | 频率:1600.000 | 二级缓存:56320 KB | Bogomips:6385.31 x4'?><!---//更改点--->
    <td width="87%" colspan="5"><?php echo $sysInfo2['cpu']['model'];?></td><!---//更改点--->
